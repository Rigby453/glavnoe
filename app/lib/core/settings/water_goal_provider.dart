...
///
/// Формула: weightKg × 30 мл × множитель активности × поправка на рост × поправка на возраст.
/// Базовый коэффициент 30 мл/кг соответствует общепринятому диапазону 30–35 мл/кг.
...
/// Результат округляется до 100 мл, зажимается в диапазон [1500, 3000].
/// Верхний потолок 3000 мл — физиологически разумный максимум для большинства людей
/// без специальной спортивной нагрузки.
...
int recommendedWaterMl({
  ...
  // Базовый расчёт: 30 мл на кг веса (стандарт 30–35 мл/кг)
  final raw = weightKg * 30.0 * multiplier * heightFactor * ageFactor;
  // Округление до 100 мл
  final rounded = ((raw / 100).round() * 100).toInt();
  // Ограничиваем физиологически разумным диапазоном; потолок 3000 мл
  return rounded.clamp(1500, 3000);
}
