...
- [ ] Тайминги анимаций: UI-переходы 120–300мс, нигде не дольше (обновить ANIMATIONS.md)

### Баги
- [ ] add_task_sheet: при клавиатуре серые треугольные артефакты по бокам скруглений — нужен сплошной фон на всех темах
- [ ] Duration: нет ручного ввода; добавить + альтернативу «время окончания»
- [ ] Иконка щита у main-задачи непонятна — пояснить/заменить
- [ ] «Клонировать неделю» копирует только одну последнюю задачу вместо всех
- [ ] Сканер штрихкода крашится (вероятно нет android.permission.CAMERA); фонарик не включается
- [ ] Голосовой ввод: язык не привязан к системному
- [ ] Отчёт сна: только мини-диаграмма, полный отчёт не открывается → сделать полный экран
- [ ] Water: нет полного отчёта → полноэкранный стакан (см. функции)
- [ ] Дневник: оценки дней/мысли не отображаются за прошлые даты → просмотр истории
- [~] Food search «connection refused» + премиум/ИИ не работают — диагностика: телефон должен собираться через run-phone.ps1 при запущенном бэкенде; проверить вместе
...
- [ ] Дыхание: авто-переключение фаз с текстом (частично есть — проверить против пожелания)
- [ ] Заморозка стрика: копирайт «Give yourself a day off» / «Chill 😌»
- [ ] Трекер привычек: хорошие с прогрессом + счётчик плохих («закурил — +1»)
- [ ] Реферальная программа: больше друзей → длиннее бесплатный период; трекинг кто кого пригласил
- [ ] Лимит 3 main-задачи: объяснить в UI или сделать настраиваемым
- [ ] Кнопка «Сообщить об ошибке / отзыв»
- [ ] Задачи из шаблонов/готовых примеров
- [ ] Глобальный undo случайных действий
- [ ] Хештеги + поиск по задачам
- [ ] Фото/видео к задачам и записям (решить хранение)
- [ ] Адаптивная вёрстка (все размеры/ориентации)
- [ ] Аналитика образа жизни («могли прочитать книгу», влияние сна на концентрацию)
- [ ] Пользовательское соглашение + дисклеймер
- [ ] Выбор даты тапом через календарь
- [ ] Просмотр задач/записей за прошлые даты
- [ ] Лимит времени в сторонних приложениях (Shorts и т.п.)
