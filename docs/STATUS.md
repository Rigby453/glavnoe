...
- [ ] **БЛОКЕР: восстановление пароля не работает на проде — нет отправки email.** Экран «Забыли
  пароль» + логика кода/сброса есть (`forgot_password_screen.dart`, `backend/src/routes/auth-reset.ts`),
  но `auth-reset.ts:62` — `TODO: отправить email через SMTP/SES (заблокировано отсутствием SMTP)`.
  В dev код возвращается в ответе (`dev_code`), на проде НЕ доставляется → реальный юзер не получит
  код и не восстановит аккаунт. Нужно: (1) подключить транзакционный email-провайдер (бесплатные
  лимиты есть, в отличие от СМС) — РАЗОВАЯ настройка пользователя (аккаунт+ключ, как Gemini/Render);
  (2) дописать отправку письма в forgot-password. Деплой на main→Render.
...
- [ ] **ИИ-итоги недели (Wrapped) просто пересказывают цифры.** `/ai/wrapped-summary` перечисляет то,
  ...
  вместо смысла. Единый принцип: «ИИ говорит то, чего пользователь сам не видит». Сфоткал черничный йогурт →
...
- [ ] **Тап по блюду в моём меню → показать и редактировать КБЖУ/сахар/клетчатку.** Хочу нажать на
  залогированное блюдо (напр. «пирожное картошка с карамелью»), увидеть посчитанные значения
  (калории/Б/Ж/У/сахар/клетчатка) и, если оценка неточная, поправить ИХ ВРУЧНУЮ (и сохранить как
  правку этой записи/продукта). Сейчас залогированная еда не открывается на просмотр/редактирование
  макросов. Нужен detail-шит записи: показать per-порцию + per-100g, поля редактирования, пересчёт
  итогов дня. Перекликается с «редактируемые макросы» из теста 2026-06-24.
