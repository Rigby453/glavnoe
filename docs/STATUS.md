...
- [ ] **ИИ-инсайт дневника: обрезан + непонятен охват:** инсайт (`/ai/diary-insight`) выдал обрезанный
  текст («…отлич.» — оборвалось) и непонятно, ЧТО он анализирует (похоже, 1 день). Фикс: (1) поднять
  лимит токенов ответа / убрать обрезку при рендере; (2) анализировать тренды за период (настроение/
  сон/вода/выполнение за неделю-месяц), не одну запись; (3) подписать, на чём основан инсайт. NB:
  бэкенд на Render собран из main — возможно тестируется старый промпт; проверить и night-версию.
- [ ] **БАГ: ИИ-фото еды — нельзя выбрать распознанное совпадение.** Сфоткал черничный йогурт →
