...
- [ ] **Периодическая задача не отображается нигде:** создана повторяющаяся задача «тренировка»
  (все дни кроме выходных, до 2030 г.) с локацией и своим цветом — не видно ни в «Сегодня», ни в
  Плане. Подозрения: разворот повторения (recurrence engine) не генерит вхождения для правила
  «будни до 2030», либо своя метка цвета/локация ломают сохранение/рендер. ВЫСОКИЙ приоритет —
  повторяющиеся задачи это ядро планера. Проверить add_task_sheet → запись recurrenceRule →
  expandedRangeItemsProvider/expandedDayItemsProvider → фильтры отображения.
